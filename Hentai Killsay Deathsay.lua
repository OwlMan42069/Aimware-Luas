...
callbacks.Register( "Draw", "handleUpdates", function()
	...
	if (update_available and not update_downloaded) then
		...
		file.Write(SCRIPT_FILE_NAME, new_version_content)
...
--------GUI Stuff--------
local tab = gui.Tab(gui.Reference("Misc"), "RetardAlert", ("ThighHighs.club v" .. VERSION_NUMBER));
...
local function CopyMessages( msg )
	if copy_chat:GetValue() and msg:GetID() == 6 then
...
callbacks.Register("DispatchUserMessage", CopyMessages);
